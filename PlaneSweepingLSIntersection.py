...
class Event(Point):
    def __init__(self, p=None, x=0, y=0):
        if p is not None and isinstance(p, Point):
    ...
    def __lt__(self, other):
        if isinstance(other, NodeValue):
            if (
                ...
                and other.seq is not None
                and self.ls is not None
                and other.ls is not None
            ):
                xs, xo = self.ls.finv(other.e), other.ls.finv(other.e)
                return (xs < xo and abs(xs - xo) > 1e-03) or (
                    abs(xs - xo) <= 1e-03 and self.seq < other.seq
                )
            else:
                if other.ls is not None:
                    xs, xo = self.ls.finv(other.e), other.ls.finv(other.e)
                else:
    ...
    def __eq__(self, other):
        if isinstance(other, NodeValue):
            if (
                ...
                if other.ls is not None:
                    return self.ls.p1 == other.ls.p1 and self.ls.p2 == other.ls.p2
                else:
                    xs, xo = self.ls.finv(other.e), other.e.x
...
class Node:
    ...
    def insert(self, value):
        if isinstance(value, NodeValue) and self.value is not None:
            if value.seq is None or value.ls is None:
                raise Exception("inserting node must have sequence and line segment")
            if self.value > value:
                if self.leftNode is not None:
                    self.leftNode.insert(value)
                else:
                    self.leftNode = Node(value, self)
            elif self.value < value:
                if self.rightNode is not None:
                    self.rightNode.insert(value)
                else:
                    self.rightNode = Node(value, self)

    def query(self, e, ls=None, res_eq=[], res_lr={"sl": None, "sr": None}):
        if ls is None:
            nv = NodeValue(e)
        else:
            nv = NodeValue(e, None, ls)
        if self.value > nv:
            res_lr["sr"] = self.value.ls
            if self.leftNode is not None:
                self.leftNode.query(e, ls=ls, res_eq=res_eq, res_lr=res_lr)
        elif self.value < nv:
            res_lr["sl"] = self.value.ls
            if self.rightNode is not None:
                self.rightNode.query(e, ls=ls, res_eq=res_eq, res_lr=res_lr)
        else:
            if ls is None:
                res_eq.append(self)
...
class BinaryTree:
    ...
    def insert(self, e, Se):
        ...
        bp = e.y - 0.5
        for ls in Se:
            if not ls.isHorizontal() and ls.bottomPoint().y > bp:
                bp = ls.bottomPoint().y
        for ls in Se:
        ...
        for i in range(lim - 1, -1, -1):
            ...
            if self.rootNode is not None:
                self.rootNode.insert(nv)
            else:
                self.rootNode = Node(nv, None)
    ...
    def query(self, e, ls=None):
        ...
        if self.rootNode is not None:
            ...
            for n in res_eq:
                if n.value.ls.isLowerPoint(e):
                    Le.append(n.value.ls)
                if n.value.ls.isInteriorPoint(e):
                    Ce.append(n.value.ls)
    ...
    def _remove(self, n):
        if n.leftNode is not None:
            ...
            if tn.leftNode is not None:
                tn.leftNode.parent = p
            ...
            if tn.rightNode is not None:
                tn.rightNode.parent = p
        else:
            if n.parent is None:
                self.rootNode = None
            else:
