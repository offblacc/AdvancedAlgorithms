...
class IntervalTree:
    ...
    def _create(self, I):
        if len(I) == 0:
            ...
            epoints = set(map(lambda si: si[0][0], I)).union(
                set(map(lambda si: si[0][1], I))
            )
            xmed = median(epoints)
            n = Node(xmed)
    ...
    def IntervalQuery(self, qx1, qx2, qy1, qy2):
        if self.rootNode is not None:
            return self._IntervalQuery(self.rootNode, qx1, qx2, qy1, qy2)
        else:
            return []
