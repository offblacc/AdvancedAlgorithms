...
class PriorityTree:
    ...
    def _create(self, Pp):
        ...
        if len(Pp) > 0:
            ...
            if len(Pl) > 0:
                n.leftChild = self._create(Pl)
            Pr = list(filter(lambda it: it[1] > ymed, Pp))
            if len(Pr) > 0:
                n.rightChild = self._create(Pr)
        return n
    ...
    def Query(self, qx2, qy1, qy2):
        ...
        for n in rn:
            if n.p[0] <= qx2 and isInRange(qy1, n.p[1], qy2):
                rv.append(n.p)
        if nsplit is None:
            return rv
        if nsplit.leftChild is None and nsplit.rightChild is None:
            ...
            if nsplit.p[0] <= qx2 and isInRange(qy1, nsplit.p[1], qy2):
                rv.append(nsplit.p)
            n = nsplit.leftChild
            while n is not None and n.p[0] <= qx2:
                if isInRange(qy1, n.p[1], qy2):
                    rv.append(n.p)
