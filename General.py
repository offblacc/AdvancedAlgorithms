...
def FindCyclesMatrix_r(G, P, u, visited, S, cycs):
    ...
    for v in n:
        if v not in S:
            ...
            if (
                P[u][v] != 1
            ):  # prevents turning back to the previous vertex in undirected graphs
                c = []
...
def FindCycleWithWeightsMatrix(G):
    ...
    if len(c) > 0:
        ...
        for cpi in cp:
            E2 = E2 | dict(
                filter(lambda x: x[0][0] in cpi and x[0][1] in cpi, E.items())
            )
    return E2
...
def GenerateRandomMatrix(nodes: list, start: chr) -> dict:
    ...
    while True:
        ...
        while not stop:
            ...
            stop = checks == clds
            for tvl in vls:
                if G[nodes[svl]][nodes[tvl]] != 0 or G[nodes[tvl]][nodes[svl]] != 0:
                    stop = False
            if not stop:
                vls.clear()
        if len(w1) > 1:
            ...
            while not stop:
                ...
                if G[nodes[s1]][nodes[t1]] == 0 and G[nodes[t1]][nodes[s1]] == 0:
                    stop = True
            w = randint(-10, 20)
            if w == 0:
                w = 21
...
class StringPL:
    def __init__(self, s: str, p: int = 0, l: int = None):
        ...
        if self.s is not None and self.p > len(self.s):
            raise "cloning position greater than the length of the string"
        if self.s is not None and self.p + self.l > len(self.s):
            raise "cloning position+length greater than the length of the string"
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, slice):
            ...
            if item > len(self):
                raise "item position greater than the length of the string"
