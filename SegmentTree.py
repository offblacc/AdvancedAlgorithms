...
class Interval:
    ...
    def inside(self, v):
        if (self.v1 is None or self.v1 <= v) and (self.v2 is None or v <= self.v2):
            return True
        return False
    ...
    def containsEndpointsOf(self, ls):
        ...
        if type(ls) is LineSegment:
            if self.inside(ls.p1.x):
                ins = ins + 1
            if self.inside(ls.p2.x):
                ins = ins + 1
            if self.containedIn(ls) or ls.containedIn(self):
                ins = ins + 2
        if type(ls) is VerticalTreeValue:
            if self.inside(ls.value[0]):
                ins = ins + 1
            if self.inside(ls.value[1]):
                ins = ins + 1
        if type(ls) is Interval:
            if self.inside(ls.v1):
                ins = ins + 1
            if self.inside(ls.v2):
                ins = ins + 1
        if type(ls) is tuple:
            if self.inside(ls[0]):
                ins = ins + 1
    ...
    def intersection(self, Iother):
        ...
        if tmpInt is not None:
            ...
            if (ox1 is not None and tx2 is not None and ox1 > tx2) or (
                ox2 is not None and tx1 is not None and ox2 < tx1
            ):
                return None
            if tx1 is None and ox1 is None:
                rx1 = None
            elif tx1 is None and ox1 is not None:
                rx1 = ox1
            elif tx1 is not None and ox1 is None:
                rx1 = tx1
            else:
                rx1 = max(tx1, ox1)
            if tx2 is None and ox2 is None:
                rx2 = None
            elif tx2 is None and ox2 is not None:
                rx2 = ox2
            elif tx2 is not None and ox2 is None:
                rx2 = tx2
            else:
                rx2 = min(tx2, ox2)
...
class VerticalTreeValue:
    ...
    def __init__(self, I, ls):
        ...
        if ls.dx == 0:
            y1, y2 = ls.p1.y, ls.p2.y
        self.value = (y1, y2, ls)

    def __lt__(self, other):
        if isinstance(other, VerticalTreeValue):
...
class VerticalTree(AVLTree):
    ...
    def _reportSubtree(self, rv, n):
        ...
        if n.L is not None:
            rv = rv + self._reportSubtree(rv, n.L)
        if n.R is not None:
            rv = rv + self._reportSubtree(rv, n.R)
        return rv
...
class SegmentTree:
    ...
    def __init__(self, ls):
        pts = set()
        for _ls in ls:
            pts = pts.union([_ls.p1.x, _ls.p2.x])
        pts = sorted(pts)
        ei, pt1 = [], None
        for pt2 in pts:
    ...
    def _medium(self, ei):
        ...
        if lei > 0:
            if ei[0].v1 is None:
                ...
                bds.append(ei[0].v1)
            if ei[lei - 1].v2 is None:
                bds.append(ei[lei - 1].v1)
            else:
                bds.append(ei[lei - 1].v2)
        return bds

    def _insertSegments(self, ls):
        for _ls in ls:
            self._insertSegment(self.rootNode, _ls)
    ...
    def _insertSegment(self, n, ls):
        if n.I.containedIn(ls):
            ...
            if n.leftChild is not None:
                nl = n.leftChild
                if nl.I.intersection(ls) is not None:
                    self._insertSegment(nl, ls)
            if n.rightChild is not None:
                nr = n.rightChild
                if nr.I.intersection(ls) is not None:
                    self._insertSegment(nr, ls)
    ...
    def _SQReportSubtree(self, n, VIq):
        rv = []
        if n.T is not None:
            rv = rv + n.T.query(n.I, VIq)
        if n.leftChild is not None:
            rv = rv + self._SQReportSubtree(n.leftChild, VIq)
        if n.rightChild is not None:
            rv = rv + self._SQReportSubtree(n.rightChild, VIq)
        return rv
