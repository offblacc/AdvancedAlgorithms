...
def FindSplittingNode(tau, mu1, mu2):
    ...
    while not isinstance(n, Value) and (
        (mu1 is not None and mu1 >= n.value) or (mu2 is not None and mu2 <= n.value)
    ):
        if mu2 <= n.value:
...
def OneDRangeQuery(tau, axis, mu1, mu2):
    ...
    if isinstance(nsplit, Value):
        if nsplit.isInRangeAxis(axis, mu1, mu2):
            rv.append(nsplit.value)
    else:
        n = nsplit.leftChild
...
class Value(Node):
    ...
    def isInRange(self, qx1=None, qx2=None, qy1=None, qy2=None):
        if not self.isInRangeAxis(0, qx1, qx2):
            return False
        if not self.isInRangeAxis(1, qy1, qy2):
            return False
        return True

    def isInRangeAxis(self, axis, mu1, mu2):
        for v in self.value:
            if mu1 is not None and v["point"][axis] < mu1:
                return False
            if mu2 is not None and v["point"][axis] > mu2:
                return False
        return True
...
class BinaryTreeWithLeafValues:
    ...
    def TwoDRangeQuery(self, x1, x2, y1, y2):
        ...
        if nsplit is None:
            return rv
        if isinstance(nsplit, Value):
            if nsplit.isInRange(x1, x2, y1, y2):
                rv.append(nsplit.value)
        else:
            n = nsplit.leftChild
            while n is not None and not isinstance(n, Value):
