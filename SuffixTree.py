...
class SuffixTree:
    ...
    def _insert1(self, prefix: StringPL, cins: chr):
        self.lastChar, self.lastTransChar, self.skipNodes, self.lastInternal = (
            -1,
            -2,
            set(),
            None,
            ...
            while cn == self.root or not cn.isLeaf():
                ...
                if cnc is not None:
                    ...
                    if s[sc : sc + minlen] == cnc.str[0:t_l]:
                        ...
                        for i in range(1, minlen):
                            if s[sc + i] != cnc.str[i]:
                                raise s + " cannot be found in the Suffix Tree"
                        if cnc.str[i + 1] != cins:
                            cnins = SuffixTreeNode(
                                cnc.str.clone(cnc.str.p, i + 1), False
                            )
                            cnc.str.p, cnc.str.l = cnc.str.p + (i + 1), cnc.str.l - (
                                i + 1
                            )
                            cn.insert(cnins)
                            cnins.insert(cnc)
                            ...
                            if len(cnins.str) == 1:
                                cnins.suffixLink = self.root
                            elif len(cnins.str) > 1:
                                self.lastInternal = cnins
                        break
                else:
