...
class BTNode:
    ...
    def _split(self) -> BTNode:
        if self.keys() > (self.deg - 1):
            i = self.minKeys() * 2 + 1
            nl, nv, nr = (
                BTNode(L=self.L[:i], deg=self.deg, P=self.P),
                self.L[i],
                BTNode(L=self.L[i + 1 :], deg=self.deg, P=self.P),
            )
            if self.P is None:
                ...
                for index in range(len(self.P.L)):
                    if self.P.L[index] is self:
                        self.P.L = (
                            self.P.L[:index] + [nl, nv, nr] + self.P.L[index + 1 :]
                        )
                        return self.P._split()
        return None

    def _rightMost(self) -> (BTNode, int):
        if self.L[-1] is not None:
            return self.L[-1]._rightMost()
        else:
            return (self, self.L[-2])
        ...
        if self.P is not None:
            ...
            for index in range(len(self.P.L)):
                if self.P.L[index] is self:
                    ...
                    if 0 <= ipr < len(self.P.L):
                        p, pi = self.P.L[ipr], index - 1
                    if 0 <= isu < len(self.P.L):
                        s, si = self.P.L[isu], index + 1
                    break
            return (p, pi, s, si)
    ...
    def _redistribute(self, s: BTNode, ci: int, pred: bool = True) -> BTNode:
        if pred:
            cmn = BTNode(L=s.L + [self.P.L[ci]] + self.L, deg=self.deg)
        else:
            cmn = BTNode(L=self.L + [self.P.L[ci]] + s.L, deg=self.deg)
        center = self.minKeys() * 2 + 1
        self.P.L[ci] = cmn.L[center]
        nl, nr = BTNode(L=cmn.L[:center], deg=self.deg, P=self.P), BTNode(
            L=cmn.L[center + 1 :], deg=self.deg, P=self.P
        )
    ...
    def _merge(self, s: BTNode, ci: int, pred: bool = True) -> BTNode:
        if pred:
            cmn = BTNode(L=s.L + [self.P.L[ci]] + self.L, deg=self.deg, P=self.P)
        else:
            cmn = BTNode(L=self.L + [self.P.L[ci]] + s.L, deg=self.deg, P=self.P)
        self.P.L[ci - 1] = cmn
    ...
    def _consolidate(self) -> BTNode:
        ...
        if self.keys() < self.minKeys():
            ...
            if p is not None:
                sib, sib_i, sib_p = p, pi, True
            else:
                sib, sib_i, sib_p = s, si, False
            if sib is not None:
                tot = sib.keys() + 1 + self.keys()
                if tot > self.deg - 1:
                    n = self._redistribute(sib, sib_i, sib_p)
                else:
                    n = self._merge(sib, sib_i, sib_p)
                if n is not None:
...
def testNode(n1: BTNode, n2: BTNode) -> (bool, str):
    if len(n1.L) != len(n2.L):
        return (
            False,
            "Duljina čvora nije ispravna... fale pokazivači, vrijednosti!? Ispravni čvor:{}, Vaš čvor:{}".format(
                n1, n2
            ),
        )
    for index in range(len(n1.L)):
        if type(n1.L[index]) is int:
            ...
            if v1 != v2:
                return (
                    False,
                    "Vrijednost {} nije na ispravnom mjestu! Ispravni čvor:{}, Vaš čvor:{}".format(
                        v2, n1, n2
                    ),
                )
            if n1.L[index - 1] is None and n2.L[index - 1] is not n1.L[index - 1]:
                return (
                    False,
                    "Greška u pokazivaču lijevog djeteta za vrijednost {}. Pokazivač treba biti None! Ispravni čvor:{}, Vaš čvor:{}".format(
                        v2, n1, n2
                    ),
                )
            if n1.L[index - 1] is not None and type(n2.L[index - 1]) is not BTNode:
                return (
                    False,
                    "Greška u pokazivaču lijevog djeteta za vrijednost {}. Treba postojati pokazivač, ovo je unutarnji čvor! Ispravni čvor:{}, Vaš čvor:{}".format(
                        v2, n1, n2
                    ),
                )
